...
router.post('/v11/TDAE-add-feedback-note', (req, res) => {

    req.session.data['feedback-note'] = 'yes'

    res.redirect('/lsf-portal/v11/TDAE-student-claim')

})

// COSA Maternity comments
router.post('/v11/COSA-status-reason', (req, res) => {

    res.redirect('/lsf-portal/v11/COSA-student-emailed')

})


// COSA Maternity comments
router.post('/v11/COSA-maternity-comments', (req, res) => {

    res.redirect('/lsf-portal/v11/COSA-maternity-confirmation')

})

// COSA Maternity dates
router.post('/v11/COSA-maternity-dates', (req, res) => {

    res.redirect('/lsf-portal/v11/COSA-maternity-comments')

})

// Return to student decline or return
router.post('/v11/COSA-student-emailed', (req, res) => {

    const location = req.session.data['location']

    if (location === 'confirmation') {
        res.redirect('/lsf-portal/v11/COSA-students-waiting-confirmation')
    } else {
        res.redirect('/lsf-portal/v11/COSA-students-declined')
    }

})

// Return to student decline or return
router.post('/v11/COSA-student-status-change', (req, res) => {

    const status = req.session.data['student-status']
    const location = req.session.data['location']

    if (status === 'In attendance' && location === 'declined') {
        res.redirect('/lsf-portal/v11/COSA-status-reason')
    } else if (status === 'Discontinued' || status === 'Not enrolled' || status === 'In attendance') {
        res.redirect('/lsf-portal/v11/COSA-student-emailed')
    } else if (status === 'Interruption') {
      res.redirect('/lsf-portal/v11/interruption')
    } else if (status === 'Information mismatch') {
      res.redirect('/lsf-portal/v11/COSA-information-mismatch-comment')
    } else {
        res.redirect('/lsf-portal/v11/COSA-maternity-dates')
    }

})

// Claims list
router.get('/v11/process-claim', (req, res) => {
...
// Return to student decline or return
router.post('/v11/returnordecline', (req, res) => {

    const returnstudent = req.session.data['return-student']

    if (returnstudent === 'Decline claim') {
        res.redirect('/lsf-portal/v11/TDAE-details-for-return')
    } else {
        res.redirect('/lsf-portal/v11/TDAE-return-reasons')
    }
...
router.post('/v11/additionalfundingexpectation', (req, res) => {

    const additionalfundingexpectation = req.session.data['additionalfundingexpectation']

    if (additionalfundingexpectation === 'yes') {
        res.redirect('/lsf-portal/v11/change-additional-funding/expectation-comment')
    } else {
        res.redirect('/lsf-portal/v11/change-additional-funding/comment')
    }

})

// CHANGE REQUEST
router.post('/v11/change-request', (req, res) => {
    ...
    if (changeRequestType === 'interruption') {
        ...
        res.redirect('/lsf-portal/v11/change-additional-funding/dates')
    } else if (changeRequestType === 'attendance-status') {
        res.redirect('/lsf-portal/v11/change-attendance-status')
    } else if (changeRequestType === 'resume-student') {
        res.redirect('/lsf-portal/v11/resume-student')
    } else if (changeRequestType === 'course-details') {
        res.redirect('/lsf-portal/v11/change-course-details')
    } else if (changeRequestType === 'student-query') {
        res.redirect('/lsf-portal/v11/general-student-query')
    } else {
        res.redirect('/lsf-portal/v11/change-request')
    }
...
router.post('/v11/TDAE-accommodation-check', (req, res) => {
    ...
    if (accommodationCheck === 'yes') {
        ...
        res.redirect('/lsf-portal/v11/TDAE-accommodation-reason')
    }

})

router.post('/v11/TDAE-accommodation-check-2', (req, res) => {
...
router.post('/v11/TDAE-accommodation-travel-check', (req, res) => {
    ...
    if (accommodationTravelCheck === 'yes') {
        ...
        res.redirect('/lsf-portal/v11/TDAE-accommodation-travel-reason')
    }

})

router.post('/v11/TDAE-accommodation-travel-check-2', (req, res) => {
...
router.post('/v11/TDAE-evidence-match', (req, res) => {
  ...
  if (evidenceMatch === 'yes') {
      ...
      res.redirect('/lsf-portal/v11/TDAE-evidence-reason')
  }
...
router.post('/v11/TDAE-evidence-match-2', (req, res) => {
...
router.post('/v11/TDAE-checked-hire-vehicle', (req, res) => {

    const vehicleHireCheck = req.session.data['vehicle-hire-check']

    if (vehicleHireCheck === 'yes') {
      res.redirect('/lsf-portal/v11/TDAE-checked-hire-vehicle-comments')
    } else {
      res.redirect('/lsf-portal/v11/TDAE-student-claim')
    }
  ...
  router.post('/v11/TDAE-checked-hire-vehicle-2', (req, res) => {
...
router.post('/v11/TDAE-checked-hire-vehicle-comments', (req, res) => {

    res.redirect('/lsf-portal/v11/TDAE-student-claim')

})

router.post('/v11/TDAE-checked-taxi', (req, res) => {

    const taxiCheck = req.session.data['taxi-check'];

    if (taxiCheck === 'yes') {
        req.session.data['taxi-completed'] = 'completed';
        res.redirect('/lsf-portal/v11/TDAE-checked-taxi-comments');
    } else {
        req.session.data['taxi-completed'] = 'in-progress';
        res.redirect('/lsf-portal/v11/TDAE-student-claim-full');
    }

});

router.post('/v11/TDAE-checked-dsa-taxi-comments', (req, res) => {

    res.redirect('/lsf-portal/v11/TDAE-student-claim-dsa')

})

router.post('/v11/TDAE-checked-dsa-taxi', (req, res) => {

    const DSATaxiCheck = req.session.data['dsa-taxi-check'];

    if (DSATaxiCheck === 'yes') {
        req.session.data['dsa-taxi-completed'] = 'completed';
        res.redirect('/lsf-portal/v11/TDAE-checked-dsa-taxi-comments');
    } else {
...
router.post('/v11/TDAE-checked-taxi-comments', (req, res) => {

    res.redirect('/lsf-portal/v11/TDAE-student-claim')

})

router.post('/v11/TDAE-overseas', (req, res) => {

    res.redirect('/lsf-portal/v11/TDAE-overseas-insurance-details')

})

router.post('/v11/TDAE-overseas-insurance-details', (req, res) => {

    res.redirect('/lsf-portal/v11/TDAE-overseas-visa-fees-details')

})

router.post('/v11/TDAE-overseas-visa-fees-details', (req, res) => {

    res.redirect('/lsf-portal/v11/TDAE-overseas-costs-summary')

})

router.post('/v11/TDAE-overseas-costs-summary', (req, res) => {

    res.redirect('/lsf-portal/v11/TDAE-checked-overseas')

})

router.post('/v11/TDAE-checked-overseas', (req, res) => {
    const overseasCheck = req.session.data['overseas-check'];

    if (overseasCheck === 'yes') {
        req.session.data['overseas-completed'] = 'completed';
        res.redirect('/lsf-portal/v11/TDAE-checked-overseas-comments');
    } else {
        req.session.data['overseas-completed'] = 'in-progress';
        res.redirect('/lsf-portal/v11/TDAE-checked-overseas-reason');
    }
});


router.post('/v11/TDAE-checked-overseas-comments', (req, res) => {
...
router.post('/v11/TDAE-checked-university-details', (req, res) => {
    ...
    if (universityMatch === 'yes') {
        ...
        res.redirect('/lsf-portal/v11/TDAE-checked-university-details-reason')
    }

})

router.post('/v11/TDAE-approve-pend-reject', (req, res) => {

    const claimOutcome = req.session.data['what-claim-outcome']

    if (claimOutcome) {
        req.session.data['approve-in-progress'] = 'in-progress'
        res.redirect('/lsf-portal/v11/TDAE-nhsbsa-comments')
    } else {
        res.redirect('/lsf-portal/v11/TDAE-approve-pend-reject')
    }

})

router.post('/v11/TDAE-nhsbsa-comments', (req, res) => {

    res.redirect('/lsf-portal/v11/TDAE-claim-declaration')

})


router.post('/v11/TDAE-claim-declaration', (req, res) => {
    ...
    if (declarationSigned) {
        ...
        res.redirect('/lsf-portal/v11/TDAE-claim-submitted')
    } else {
        res.redirect('/lsf-portal/v11/TDAE-claim-declaration')
    }
...
router.post('/v11/TDAE-send-to-student', (req, res) => {

    const returnStudent = req.session.data['return-student']

    res.redirect('/lsf-portal/v11/TDAE-return-cya')

})

router.post('/v11/TDAE-details-for-return', (req, res) => {
